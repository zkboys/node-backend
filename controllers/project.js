...
module.exports = class ProjectController {
    ...
    static async create(ctx) {
        ...
        if (project) {
            project.name === name
                ? ctx.fail(`项目 ${name} 已存在`)
                : ctx.fail('请检查 URL 是否已经存在');
            return;
        }
    ...
    static async copy(ctx) {
        ...
        if (checkProject) {
            checkProject.name === newName
                ? ctx.fail(`项目 ${newName} 已存在`)
                : ctx.fail('请检查 URL 是否已经存在');
            return;
        }
